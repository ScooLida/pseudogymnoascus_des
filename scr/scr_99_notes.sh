...
Представьте, что у нас есть два файла:

    ..._B_PE100_10g_...

    ..._B_11g_...

Логика sed с использованием ?:

    Видит первый файл: «Ага, после _B_ идет PE100_. Подходит под условие (PE100_)?. Удаляю».

    Видит второй файл: «После _B_ сразу идет 11g. Текста PE100_ нет, но у меня стоит знак ? (необязательно), значит, я просто игнорирую это и работаю с 11g».

Другие важные знаки в том коде:

    .* — означает «любое количество любых символов» (так мы «съедаем» длинную дату и ID в начале).

    \2 — это «вторая карманная переменная». Она вставляет то, что мы сохранили во вторых скобках (само название образца).
git config --global user.email your_email@example.com
git config --global user.name Lidia Tursunova
